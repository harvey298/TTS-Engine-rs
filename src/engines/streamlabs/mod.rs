...
impl Polly {
    ...
    pub async fn run(&mut self, text: &str, filename: &str, wait_on_ratelimit: bool) -> Result<()> {

        let data = self.gen_tts(text, wait_on_ratelimit).await?;
    ...
    #[async_recursion]
    pub async fn gen_tts(&mut self, text: &str, wait_on_ratelimit: bool) -> Result<Bytes> {
        ...
        let res = client.post(POLLY_URL).json(&map).send().await?;
        ...
        if code != 200 {
            if code == 429 {
                ...
                if self.ror {
                   return Ok( self.gen_tts(text, wait_on_ratelimit).await? )
